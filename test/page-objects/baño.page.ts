...
class Bano{
   ...
   async obtenerPropiedadesBaño (): Promise<PropiedadesProducto[]> {  
    ...
    async hasCompraOnline (product){
        const compraOnline = await product.$('.hide-phone').$('.pdp-availability-buy-online')
        return await compraOnline.isExisting()
    }
    async hasCompraTienda (product){
        const compraTienda = await product.$('.hide-phone').$('.pdp-availability-available-in-store')
